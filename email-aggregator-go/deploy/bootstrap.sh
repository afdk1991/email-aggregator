...
info "重放迁移 001_init.sql + 002_citus_sharding.sql + 003_read_flag.sql + 004_account_registry.sql（幂等，可重跑）…"
...
$COMPOSE exec -T postgres psql -U "$PG_USER" -d "$PG_DATABASE" -v ON_ERROR_STOP=1 \
  -f /migrations/001_init.sql -f /migrations/002_citus_sharding.sql -f /migrations/003_read_flag.sql -f /migrations/004_account_registry.sql \
  && ok "迁移完成（mail_metadata / account_sync_cursor / read 列 / mail_account 已就绪）" \
